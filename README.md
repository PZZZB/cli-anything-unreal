...
`material dump-hlsl` rejects an inactive requested shader platform before recompiling, preserves the material package's original dirty flag, and returns a nonzero structured error when no dump or matching shader stage is available. After recompilation it waits up to 10 seconds for dump files by default; use `--wait-timeout <seconds>` only when a large material is still compiling. Missing-dump details include the exact search root and candidate count.
