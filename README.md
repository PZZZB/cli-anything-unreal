...
`editor status` applies a 15-second discovery deadline by default; the root `status` alias accepts the same `--timeout <seconds>` option. An exhausted deadline returns `EDITOR_STATUS_TIMEOUT` with the `blocking_phase`; a blocked task read also includes its `task_id`. When a matching UnrealEditor process and active launch task are already known, a task-lock timeout instead retains the last fully published `launching` snapshot and marks it as potentially stale.
