...
- **Workaround (CLI):** `ue-cli` already handles this internally by using the `editor run-script` execution model to retrieve transforms via the Python API `actor.get_actor_transform()` instead of the raw property endpoints. No further action is required for users or agents.