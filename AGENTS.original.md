...
`ue-cli` is a Python CLI tool that lets AI coding agents control Unreal Engine 5 editors. It wraps UE's Remote Control HTTP API and UAT/UBT build tools behind structured, token-efficient CLI commands with JSON output.
...
- Worker runs `ue-cli _task-worker run <task_id>` (hidden Click command).
- CLI caller polls with `task status <task_id>`. Final statuses: `completed`, `failed`, `timeout`, `cancelled`.

### Skill System (`skills/`)

`SKILL.md` is the Codex skill definition — it tells the AI agent how to use the CLI. The `references/` directory contains per-domain docs loaded on demand to save context window. The `install-skills` command copies these into the user's Codex settings.
