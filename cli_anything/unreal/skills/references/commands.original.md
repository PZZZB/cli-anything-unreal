...
Complete command reference for `ue-cli`. Output defaults to JSON for non-TTY callers; use `--output json` explicitly to force it in a terminal.
...
| `editor viewport bookmark jump --index N [--timeout SEC]` | Jump Level Viewport to bookmark 0-9 using the editor numeric shortcut; Windows only | Yes |
