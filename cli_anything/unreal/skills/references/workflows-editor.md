...
- `editor launch` starts an interactive editor by default. Pass `--unattended` only when UE dialogs must be suppressed; `--no-unattended` explicitly preserves interactivity. It waits until the API is online, then returns `launching` with a task id after a bounded foreground wait if startup is still in progress. `--timeout` controls the background startup deadline and does not extend the foreground shell wait. Do not use `sleep`.
