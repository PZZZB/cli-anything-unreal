...
| `editor launch [--map /Game/PATH] [--no-wait] [--timeout N] [--unattended/--no-unattended]` | Prepare Remote Control/bridge project files when needed, then launch editor; waits until ready and kills zombies. Launches an interactive editor by default; `--unattended` suppresses UE dialogs and `--no-unattended` explicitly preserves interactivity. Map package paths must include their mount root; bare names such as `Oregon_Main` are rejected. Foreground wait is always bounded so shells do not kill the command; if the editor is still starting, the command returns a pollable `launching` task while `--timeout` remains the background startup deadline. | No |
