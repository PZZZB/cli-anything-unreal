...
| `editor launch [--map /Game/PATH] [--no-wait] [--timeout N] [--unattended/--no-unattended]` | Prepare Remote Control/bridge project files when needed, then launch editor; waits up to 30 seconds for readiness and kills zombies. `editor start` is an exact alias with the same options. Launches an interactive editor by default; `--unattended` suppresses UE dialogs and `--no-unattended` explicitly preserves interactivity. Map package paths must include their mount root; bare names such as `Oregon_Main` are rejected. If the editor is still starting, the command returns a pollable `launching` task while `--timeout` remains the background startup deadline. | No |
