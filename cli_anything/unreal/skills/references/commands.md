...
| `editor open-level /Game/PATH [--timeout SEC]` | Open an existing level via `LevelEditorSubsystem.LoadLevel`, then verify the active editor world. The synchronous request waits up to 300 seconds by default; timeout is configurable for large maps. `EDITOR_OPEN_LEVEL_TIMEOUT` means completion is unknown, so do not retry or launch another editor until status and active world are verified. UE4.26 lacks this subsystem; unloaded targets return `EDITOR_OPEN_LEVEL_UNSUPPORTED` with a safe close plus `editor launch --map` workflow. Rejects an already-loaded, non-active target World with `EDITOR_OPEN_LEVEL_UNSAFE_LOADED_WORLD`; close and use `editor launch --map` for that state. The path must include its Unreal mount root | Yes |
