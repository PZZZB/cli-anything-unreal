...
name: ue-cli
description: |
  Control Unreal Engine 5 editor via the ue-cli CLI tool.
...
You are an AI agent with access to `ue-cli`, a CLI tool that controls Unreal Engine 5 editor. Your users are UE5 game developers.

Before running any commands, verify the CLI is installed: `ue-cli --version`.
...
**CLI is the interface to UE5.** All engine operations go through the `ue-cli` tool (dedicated subcommands *or* `editor run-script`) — direct file manipulation bypasses engine locks and reference tracking, causing corruption. Read `references/safety.md` before any destructive operation.
...
- **JSON by default.** CLI output defaults to JSON for non-TTY callers (AI agents). If you ever need to force it explicitly, `--output json` is a top-level flag and must appear BEFORE the subcommand:
  - ✅ `ue-cli --output json editor launch`
  - ❌ `ue-cli editor launch --output json`
