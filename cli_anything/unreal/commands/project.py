"""commands/project.py — Project management commands."""
...
def config_list(state: AppState):
    ...
    configs = list_configs(state.session.project_dir)
    output(configs, state)
...
def config_set(state: AppState, config_name, section, key, value):
    ...
    result = set_config(state.session.project_dir, config_name, section, key, value)
    output(result, state)
...
def project_generate(state: AppState):
    ...
    require_project(state)
    result = generate_project_files(state.session.project_path, state.session.engine_root)
    output(result, state)
